...
const p=PERSONAS.find(p=>p.id===process.argv[2]) || PERSONAS[0];
const maxSteps=Math.min(30,Math.max(1,Number(process.argv[3])||12));
const run={id,url:'https://markerpad.app/',mode:'codex',model:'gpt-6-astra',recordVideo:true,status:'running',createdAt:new Date().toISOString(),maxSteps,issues:[],ghosts:[{id:'ghost-1',persona:p.id,name:p.name,label:p.label,status:'queued',steps:[],findings:[],usage:{input_tokens:0,output_tokens:0}}]};
